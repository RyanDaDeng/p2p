    ...
    >
        <template #content>
            <form @submit.prevent class="space-y-6">
                <!-- Trade Type -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">交易类型</label>
                    <div class="grid grid-cols-2 gap-3">
                        ...
                        >
                            ...
                            <div class="text-center">
                                ...
                                <p class="font-medium text-gray-900 dark:text-slate-100">商家出售</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">向用户出售虚拟货币</p>
                            </div>
                        ...
                        >
                            ...
                            <div class="text-center">
                                ...
                                <p class="font-medium text-gray-900 dark:text-slate-100">商家收购</p>
                                <p class="text-xs text-gray-500 dark:text-slate-400 mt-1">从用户手中收购虚拟货币</p>
                            </div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">虚拟货币 <span class="text-red-500">*</span></label>
                        <div class="relative" ref="currencyDropdownRef">
                            <button
                                type="button"
                                @click="showCurrencyDropdown = !showCurrencyDropdown"
                                class="w-full px-3 py-2 pr-10 text-left bg-white dark:bg-slate-800 border border-gray-300 dark:border-slate-600 rounded-lg hover:border-gray-400 dark:hover:border-slate-500 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all duration-200 cursor-pointer"
                            >
                                <span class="flex items-center">
                                    <component v-if="form.currency && getCryptoIcon(form.currency)" :is="getCryptoIcon(form.currency)" :size="20" class="mr-2" />
                                    <span :class="form.currency ? 'text-gray-900 dark:text-slate-100' : 'text-gray-500 dark:text-slate-400'">
                                        {{ form.currency ? currencies.find(c => c.value === form.currency)?.label : '选择币种' }}
                                    </span>
                                </span>
                                <svg class="absolute right-3 top-3 h-4 w-4 text-gray-500 dark:text-slate-500 transition-transform duration-200" 
                            ...
                            >
                                <div
                                    ...
                                    class="absolute z-50 mt-2 w-full rounded-xl bg-white dark:bg-slate-800 shadow-xl ring-1 ring-gray-200 dark:ring-slate-700/50"
                                >
                                    <div class="px-3 py-2 border-b border-gray-200 dark:border-slate-700/50">
                                        <p class="text-xs font-medium text-gray-500 dark:text-slate-400">选择币种</p>
                                    </div>
                                    ...
                                    <div class="max-h-60 overflow-auto py-1 custom-scrollbar">
                                        <button
                                            ...
                                            :class="[
                                                'w-full text-left px-3 py-2 text-sm hover:bg-gray-50 dark:hover:bg-slate-700 transition-colors cursor-pointer flex items-center justify-between',
                                                option.value === form.currency ? 'bg-emerald-50 dark:bg-emerald-900/20 text-emerald-700 dark:text-emerald-400' : 'text-gray-700 dark:text-slate-300'
                                            ]"
                                        >
                                            <span class="flex items-center">
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">交易货币 <span class="text-red-500">*</span></label>
                        <P2PSelect 
                    ...
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">国家 <span class="text-red-500">*</span></label>
                        <P2PSelect
                ...
                <!-- Price Settings -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">价格设置 <span class="text-red-500">*</span></label>
                    <div class="space-y-3">
                        <div class="flex gap-3">
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" v-model="form.priceType" value="fixed" class="mr-2 text-emerald-600 focus:ring-emerald-500" />
                                <span class="text-sm text-gray-700 dark:text-slate-300">固定价格</span>
                            </label>
                            <label class="flex items-center cursor-pointer">
                                <input type="radio" v-model="form.priceType" value="floating" class="mr-2 text-emerald-600 focus:ring-emerald-500" />
                                <span class="text-sm text-gray-700 dark:text-slate-300">浮动价格</span>
                            </label>
                        </div>
                        ...
                        <div v-if="form.priceType === 'fixed'" class="flex items-center gap-2">
                            <input 
                                ...
                                step="0.01"
                                placeholder="0.00"
                                class="flex-1 px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                            />
                            <span class="text-gray-500 dark:text-slate-400">{{ form.fiatCurrency || 'CNY' }}</span>
                        </div>
                ...
                <!-- Limits -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">交易限额 <span class="text-red-500">*</span></label>
                    <div class="grid grid-cols-2 gap-3">
                        <div>
                            <div class="relative">
                                <input 
                                    ...
                                    type="number" 
                                    placeholder="最低限额"
                                    class="w-full px-3 py-2 pr-12 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                                />
                                <span class="absolute right-3 top-2.5 text-gray-500 dark:text-slate-400 text-sm">{{ form.fiatCurrency || 'CNY' }}</span>
                            </div>
                        </div>
                        <div>
                            <div class="relative">
                ...
                <!-- Payment Method -->
                <div>
                    <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">支付方式 <span class="text-red-500">*</span></label>
                ...
                <div>
                    ...
                    <input 
                        ...
                        maxlength="255"
                        class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent"
                    />
                    <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                ...
                <div class="border border-gray-200 dark:border-slate-700 rounded-lg p-4 space-y-4">
                    <div>
                        <h3 class="text-sm font-medium text-gray-900 dark:text-slate-100 flex items-center gap-2">
                    ...
                    <!-- Welcome Message -->
                    <div>
                        <label class="block text-sm font-medium text-gray-700 dark:text-slate-300 mb-2">欢迎消息</label>
                        <textarea 
                            v-model="form.welcomeMessage" 
                            rows="3" 
                            placeholder="交易开始时自动发送的消息（选填）"
                            class="w-full px-3 py-2 border border-gray-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-gray-900 dark:text-slate-100 focus:ring-2 focus:ring-emerald-500 focus:border-transparent resize-none text-sm"
                        ></textarea>
                        <p class="mt-1 text-xs text-gray-500 dark:text-slate-400">
                            当用户开始交易时自动发送的欢迎消息
                        </p>
                    ...
                    <div class="p-3 bg-blue-50 dark:bg-blue-900/20 rounded-lg border border-blue-200 dark:border-blue-800">
                        <div class="flex items-start">
                            ...
                            <p class="ml-2 text-xs text-blue-700 dark:text-blue-300">
