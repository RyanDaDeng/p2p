                        ...
                        <div v-if="title || slots.header" class="px-4 md:px-6 py-3 md:py-4 border-b border-gray-200 dark:border-gray-800">
                            <div class="flex items-center justify-between">
                                ...
                                <button
                                    ...
                                    :class="loading ? 'cursor-not-allowed opacity-50' : 'hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer'"
                                >
                            ...
                            <slot name="footer">
                                <div class="flex gap-3">
                                    <button
                                        ...
                                        @click="$emit('close')"
                                        :disabled="loading"
                                        class="flex-1 px-4 py-2.5 border-2 rounded font-medium transition-colors"
                                        :class="loading
                                            ? 'border-gray-200 dark:border-gray-700 text-gray-400 dark:text-gray-600 cursor-not-allowed opacity-50'
                                            : 'border-gray-300 dark:border-gray-600 text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-800 cursor-pointer'"
                                    >
                                        {{ cancelText }}
                                    </button>
                                    <button
                                        v-if="confirmText"
                                        @click="handleConfirm"
                                        :disabled="(requiresAgreement && !agreed) || loading"
                                        :class="[
                                            'flex-1 px-4 py-2.5 rounded font-medium transition-all relative',
                                            (!requiresAgreement || agreed) && !loading
                                    ...
                                    >
                                        ...
                                        <div v-else-if="requiresAgreement && !agreed" class="absolute inset-0 flex items-center justify-center">
                                            <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" />
                                            </svg>
                                            请先勾选同意
                                        </div>
                                        <span :class="{ 'opacity-0': (requiresAgreement && !agreed) || loading }">
                                            {{ confirmText }}
                                        </span>
                                    </button>
                                </div>
