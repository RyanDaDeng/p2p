<template>
    <P2PAppLayout>
        ...
        <div class="bg-gradient-to-r from-gray-100 to-gray-200 dark:from-slate-900 dark:to-slate-800">
            <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-6">
                ...
                <div class="flex flex-col sm:flex-row items-center sm:items-start gap-4">
                    ...
                    <img
                        ...
                        :alt="user.name"
                        class="w-16 h-16 sm:w-20 sm:h-20 rounded-full object-cover shadow-lg border-2 border-white dark:border-slate-800"
                    />
                    ...
                    <div class="flex-1 text-center sm:text-left">
                        ...
                        <div class="flex flex-wrap items-center justify-center sm:justify-start gap-3 mt-2 text-sm">
                            ...
                            <span class="text-gray-600 dark:text-slate-400">注册于 {{ user.memberSince }}</span>
                        </div>
                    </div>
                    ...
                    <div class="hidden sm:flex items-center gap-6 text-center">
                        <div>
                            ...
                            <div class="text-xs text-gray-600 dark:text-slate-400">成功交易</div>
                        </div>
                        <div>
                            ...
                            <div class="text-xs text-gray-600 dark:text-slate-400">用户评分</div>
                        </div>
                        <div>
                            <div class="text-2xl font-bold text-gray-900 dark:text-white">{{ user.usersTradedWith }}</div>
                            <div class="text-xs text-gray-600 dark:text-slate-400">交易伙伴</div>
                        </div>
                        <div>
                            <div class="text-2xl font-bold text-gray-900 dark:text-white">
                                {{ user.responseTime ? '< ' + user.responseTime + '分钟' : '暂无' }}
                            </div>
                            <div class="text-xs text-gray-600 dark:text-slate-400">响应时间</div>
                        </div>
                    </div>
                </div>
                
                <!-- Mobile Stats -->
                <div class="sm:hidden grid grid-cols-4 gap-2 mt-4 pt-4 border-t border-gray-300 dark:border-slate-700">
                    <div class="text-center">
                        <div class="text-base font-bold text-gray-900 dark:text-white">{{ user.completedTrades }}</div>
                        <div class="text-xs text-gray-600 dark:text-slate-400">交易</div>
                    </div>
                    <div class="text-center">
                        ...
                        <div class="text-xs text-gray-600 dark:text-slate-400">评分</div>
                    </div>
                    <div class="text-center">
                        <div class="text-base font-bold text-gray-900 dark:text-white">{{ user.usersTradedWith }}</div>
                        <div class="text-xs text-gray-600 dark:text-slate-400">伙伴</div>
                    </div>
                    <div class="text-center">
                        <div class="text-base font-bold text-gray-900 dark:text-white">
                            {{ user.responseTime ? '< ' + user.responseTime + '分' : '暂无' }}
                        </div>
                        <div class="text-xs text-gray-600 dark:text-slate-400">响应</div>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="bg-gray-50 dark:bg-slate-950 min-h-screen">
            <div class="mx-auto max-w-7xl px-4 py-4 sm:px-6 lg:px-8">
                ...
                <div class="bg-white dark:bg-slate-900 rounded-lg shadow-sm">
                    <!-- Header -->
                    <div class="px-4 py-3 border-b border-gray-200 dark:border-slate-800">
                        <div class="flex items-center justify-between">
                            <div class="flex items-center gap-3">
                                <h2 class="text-lg font-semibold text-gray-900 dark:text-slate-100">用户评价</h2>
                                <span class="px-2 py-1 bg-gray-100 dark:bg-slate-800 rounded text-sm text-gray-600 dark:text-slate-400">
                                    {{ user.totalReviews }} 条
                                </span>
                            </div>
                            <div class="flex items-center gap-2">
                                <div class="flex">
                                    <svg v-for="i in 5" :key="i" class="w-4 h-4" 
                                         :class="i <= Math.floor(user.rating) ? 'text-amber-400' : 'text-gray-300 dark:text-slate-700'"
                                         fill="currentColor" viewBox="0 0 20 20">
                                        <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                    </svg>
                                </div>
                                <span class="text-lg font-bold text-gray-900 dark:text-slate-100">{{ user.rating }}</span>
                            </div>
                        </div>
                    ...
                    <!-- Tabs -->
                    <div class="flex gap-1 px-4 py-2 border-b border-gray-200 dark:border-slate-800 overflow-x-auto">
                        <button v-for="tab in feedbackTabs" :key="tab.value"
                                @click="activeFeedbackTab = tab.value"
                                :class="[
                    ...
                    <div class="divide-y divide-gray-200 dark:divide-slate-800">
                        <div v-if="feedbacks.length === 0" class="p-8 text-center">
                            ...
                            <p class="text-gray-500 dark:text-slate-400">暂无评价</p>
                        </div>
                        
                        <div v-for="feedback in feedbacks" :key="feedback.id" class="p-4 hover:bg-gray-50 dark:hover:bg-slate-800/50 transition-colors">
                            <div class="flex items-start gap-3">
                                ...
                                <div class="flex-1 min-w-0">
                                    <div class="flex items-start justify-between gap-2">
                                        <div>
                                            <div class="flex items-center gap-2">
                                                <p class="text-sm font-medium text-gray-900 dark:text-slate-100">{{ feedback.userName }}</p>
                                                <span class="text-xs text-gray-500 dark:text-slate-500">{{ feedback.date }}</span>
                                            </div>
                                            <div class="flex items-center gap-1 mt-1">
                                                <svg v-for="i in 5" :key="i" class="w-3.5 h-3.5" 
                                                     :class="i <= feedback.rating ? 'text-amber-400' : 'text-gray-300 dark:text-slate-700'"
                                                     fill="currentColor" viewBox="0 0 20 20">
                                                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z"/>
                                                </svg>
                                                <span class="text-xs text-gray-600 dark:text-slate-400 ml-1">
                                                    {{ feedback.tradeType }} {{ feedback.cryptoCurrency }}
                                                </span>
                                            </div>
                                        </div>
                                    </div>
                                    <p class="mt-2 text-sm text-gray-700 dark:text-slate-300 line-clamp-2">{{ feedback.comment }}</p>
                                </div>
                            </div>
                        </div>
                    </div>
...
const feedbackTabs = computed(() => {
    const stats = props.ratingStats;
    const positive = (stats[5] || 0) + (stats[4] || 0);
    const neutral = stats[3] || 0;
    const negative = (stats[2] || 0) + (stats[1] || 0);
    const total = props.user.totalReviews || 0;
    
    return [
        { value: 'all', label: '全部', count: total },
        { value: 'positive', label: '好评', count: positive },
        { value: 'neutral', label: '中评', count: neutral },
        { value: 'negative', label: '差评', count: negative }
    ];
});
...
// Pagination state
const currentPage = computed(() => props.reviewsPagination.current_page);
const totalPages = computed(() => props.reviewsPagination.last_page);
